...
from djangonews.news.models import *
from djangonews.news.views import *
...
class BasicNewsTest(TestCase):
    ...
    def test_submit_post_with_title_and_url(self):
        ...
        request = self.factory.post(
            "/submit", {"title": "test-post-asdf", "url": "https://sebastiansteins.com"}
        )
        request.user = self.user
        response = submit(request)
    ...
    def test_submit_and_upvote(self):
        ...
        self.assertEqual(
            User.objects.get(pk=self.user.pk).karma, 0
    ...
    def test_submit_and_downvote(self):
        ...
        request = self.factory.post(
            "/submit",
            {"title": "test-post-asdf-12", "url": "https://sebastiansteins.com/12"},
        )
        request.user = self.other_user
        response = submit(request)
        item_url = response.url
    ...
    def test_ask(self):
        ...
        request = self.factory.post(
            "/submit",
            {"title": "Ask: test-post-asdf", "url": "https://sebastiansteins.com"},
        )
        request.user = self.user
        response = submit(request)
    ...
    def test_show(self):
        ...
        request = self.factory.post(
            "/submit",
            {"title": "Show: test-post-asdf", "url": "https://sebastiansteins.com"},
        )
        request.user = self.user
        response = submit(request)
    ...
    def test_comment(self):
        ...
        submission = Story(user=self.other_user, title="Test-1234", text="bla")
        submission.save()
    ...
    def test_owner_cannot_delete_comment_with_children(self):
        ...
        comment2 = Comment(
            to_story=story, user=self.user, text="another comment", parent=comment
        )
        comment2.save()
...
class ReceiversNewsTest(TestCase):
    ...
    def test_check_for_duplicates(self):
        ...
        story2 = Story(
            title="Once again", url="http://example.org", user=self.other_user
        )
        story2.save()
